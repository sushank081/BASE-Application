...
export default function MasterDefectsPage() {
  const router = useRouter();

  // Core Data & Processing States
  const [defects, setDefects] = useState<MasterDefect[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isMutationProcessing, setIsMutationProcessing] = useState(false);

  // Form Modal States
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingDefect, setEditingDefect] = useState<MasterDefect | null>(null);

  // Controlled Form Inputs
  const [defectName, setDefectName] = useState("");
  const [isActiveStatus, setIsActiveStatus] = useState("ACTIVE");

  // Fetch all defect records on component mount
  useEffect(() => {
    fetchDefects();
  }, []);

  // -------------------------------------------------------------------------
  // 1. GET METHOD: FETCH ALL DEFECTS
  // -------------------------------------------------------------------------
  const fetchDefects = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/api/v1/master/defects`, {
      ...
      if (!res.ok) {
        const errorDetail =
          typeof data?.detail === "string"
            ? data.detail
            : "Failed to pull master defect records.";
      ...
      const actualMsg =
        typeof err === "string"
          ? err
          : err?.message || "An unexpected error occurred while fetching defect list.";
      alert(`Master Defect Sync Error:\n${actualMsg}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // -------------------------------------------------------------------------
  // 2. POST & 3. PATCH METHOD: SAVE / UPDATE DEFECT
  // -------------------------------------------------------------------------
  const handleSaveDefect = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!defectName.trim() || isMutationProcessing) return;

    const isEditMode = !!editingDefect;
    setIsMutationProcessing(true);

    const targetUrl = isEditMode
      ? `${BASE_URL}/api/v1/master/defects/${editingDefect.id}`
      : `${BASE_URL}/api/v1/master/defects`;
    ...
    try {
      ...
      alert(
        `Master defect record successfully ${
          isEditMode ? "updated" : "added to registry"
        }!`
      );
      closeModalWorkflow();
      fetchDefects(); // Re-sync table state
    } catch (err: any) {
      console.error("Save Defect Error:", err);
      const actualErrorString =
        typeof err === "string"
          ? err
          : err?.message || "An error occurred while saving the defect record.";
  ...
  const openEditModal = (defect: MasterDefect) => {
    setEditingDefect(defect);
    setDefectName(defect.defect_name);
    setIsActiveStatus(defect.is_active);
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-8 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 font-sans text-black">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-emerald-800/10 pb-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-black flex items-center gap-2">
              <AlertOctagon className="w-7 h-7 text-emerald-600" /> Master Defect Registry
            </h1>
            <p className="text-xs text-black/70 mt-0.5 font-medium">
              Shop Floor Non-Conformance Defect Identification Catalog
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Plus className="w-4 h-4" /> Add Master Defect
          </button>
        </div>
      </div>

      {/* ------------------------------------------------------------------------- */}
      {/* SECTOR 1: MASTER DEFECTS TABLE (GET DISPLAY) */}
      {/* ------------------------------------------------------------------------- */}
      <div className="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl shadow-xl overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center text-black/70 font-semibold animate-pulse text-sm">
            Fetching Master Defect Records...
          </div>
        ) : defects.length === 0 ? (
          <div className="p-12 text-center text-black/50 text-sm font-medium">
            No defect categories registered inside master dataset.
          </div>
        ) : (
            ...
            <table className="w-full border-collapse text-left text-xs text-black">
              <thead className="bg-emerald-900 text-white font-bold tracking-wider uppercase text-[10px]">
                <tr>
                  ...
                  <th className="px-6 py-4">Defect Name</th>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-emerald-950/40 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-md w-full overflow-hidden text-black animate-in zoom-in-95 duration-150">
            ...
            <div className="bg-emerald-900 text-white px-6 py-4 flex items-center justify-between border-b border-emerald-950">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold tracking-tight">
                  {editingDefect
                    ? `Modify Defect Record #${editingDefect.id}`
                    : "Register New Master Defect"}
                </h2>
              </div>
              <button
            ...
            {/* Ingestion Form Body */}
            <form onSubmit={handleSaveDefect} className="p-6 space-y-4 text-black">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. BMS Communication Fault"
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-100">
                ...
                >
                  ...
                  {isMutationProcessing ? "Saving..." : "Commit Defect Entry"}
                </button>
