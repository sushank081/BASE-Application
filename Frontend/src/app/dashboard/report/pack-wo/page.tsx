...
export default function PackWorkOrdersPage() {
  const router = useRouter();

  // Core Data & Loading States
  const [workOrders, setWorkOrders] = useState<PackWorkOrder[]>([]);
  ...
  // 1. GET METHOD: Hydrate Tracking Table from Pack Work Orders Backend Endpoint
  const fetchPackWorkOrders = useCallback(async () => {
    setIsLoading(true);

    try {
      // Build dynamic URL query parameters cleanly without dangling '?' or strict slash conflicts
      ...
      const url = queryString
        ? `${BASE_URL}/api/v1/production/pack-work-orders?${queryString}`
        : `${BASE_URL}/api/v1/production/pack-work-orders`;
      ...
      // Safe Response Parsing: prevents "SyntaxError: Unexpected token 'I'" if server returns plain text error
      ...
      if (contentType.includes("application/json")) {
        ...
        data = { detail: rawText || `Server responded with status code: ${response.status}` };
      }
      ...
      console.error("Error fetching pack work orders:", err);
      alert(`PACK LEDGER RECOVERY ERROR:\n${err.message || "Failed to establish a link with database gateway."}`);
    } finally {
      setIsLoading(false);
    }
  }, [startDate, endDate, searchMaterial, statusFilter]);

  // Fetch data on initial mount and whenever filter criteria adjust
  useEffect(() => {
    fetchPackWorkOrders();
  }, [fetchPackWorkOrders]);
    ...
    <div className="space-y-6 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 font-sans text-slate-800">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2">
              <ClipboardList className="w-6 h-6 text-cyan-600" /> Battery Pack Work Orders
            </h1>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Pack Line Scheduling & Enclosure Production Ledger Tracking
            </p>
          </div>
        </div>

        <div className="bg-slate-900 text-white px-4 py-2 rounded-xl text-right shadow-md border border-slate-800 hidden sm:block">
          <span className="block text-[10px] uppercase font-bold tracking-widest text-cyan-400">Total Orders</span>
      ...
      <div className="bg-white/70 backdrop-blur-md border border-cyan-200/60 p-5 rounded-2xl shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-2">
          ...
          {(startDate || endDate || searchMaterial || statusFilter) && (
            <button
              ...
              className="text-xs font-bold text-cyan-700 hover:text-cyan-900 flex items-center gap-1 cursor-pointer transition-all"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Search Material Number</label>
            <div className="relative">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Filter by Status</label>
            <select
              value={statusFilter}
              onChange={(e) => { setStatusFilter(e.target.value); setCurrentPage(1); }}
              className="block w-full rounded-xl border border-cyan-200/80 bg-white/90 p-2 text-xs font-bold text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-500/15 transition-all cursor-pointer"
            >
      ...
      <div className="bg-white/70 backdrop-blur-md border border-cyan-200/60 rounded-2xl shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center text-slate-700 font-semibold flex items-center justify-center gap-2 text-sm">
            <Loader2 className="w-5 h-5 animate-spin text-cyan-600" /> Fetching Battery Pack Work Orders...
          </div>
        ) : paginatedOrders.length === 0 ? (
          <div className="p-12 text-center text-slate-400 text-sm font-medium italic">
            No Pack Work Orders match the specified filter criteria.
          </div>
        ) : (
            ...
            <table className="w-full border-collapse text-left text-xs text-slate-800">
              <thead className="bg-slate-900 text-slate-100 font-semibold tracking-wider uppercase text-[10px]">
                <tr>
                  ...
                  <th className="px-4 py-3.5">BOP ID</th>
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {paginatedOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-slate-600 whitespace-nowrap">{order.bop_id || "—"}</td>
                    <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{order.order_type || "—"}</td>
                    ...
                    <td className="px-4 py-3 text-[11px] font-mono text-slate-600 whitespace-nowrap">{formatDateToken(order.start_time)}</td>
                    <td className="px-4 py-3 text-[11px] font-mono text-slate-600 whitespace-nowrap">{formatDateToken(order.end_time)}</td>
      ...
      {!isLoading && workOrders.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/70 backdrop-blur-md border border-cyan-200/60 p-4 rounded-2xl shadow-sm">
          <div className="text-xs text-slate-600 font-medium">
            Showing <span className="font-bold text-slate-900">{(currentPage - 1) * ROWS_PER_PAGE + 1}</span> to{" "}
            <span className="font-bold text-slate-900">
              {Math.min(currentPage * ROWS_PER_PAGE, workOrders.length)}
            </span>{" "}
            of <span className="font-bold text-slate-900">{workOrders.length}</span> Work Orders
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="p-2 rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 disabled:opacity-40 disabled:pointer-events-none transition-all cursor-pointer shadow-xs"
                title="Previous Page"
              >
