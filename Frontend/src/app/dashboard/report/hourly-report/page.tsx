...
import { Calendar, RefreshCw, Filter, Clock, AlertCircle } from "lucide-react";
import { BASE_URL } from "../../../../utils/utilsapi";

// =============================================================================
// INLINE TYPES & INTERFACES (Eliminates @/types/hourlyReport build dependency)
// =============================================================================
export interface HourlyMaterialReportRow {
...
export interface HourlyMaterialReportResponse {
  status: string;
  report_type?: string;
  report_date?: string;
  data: HourlyMaterialReportRow[];
}
...
export default function HourlyMaterialReportTable() {
  // Calendar date state - empty/unselected by default
  const [selectedDate, setSelectedDate] = useState<string>("");

  const [reportData, setReportData] = useState<HourlyMaterialReportRow[]>([]);
  ...
  const fetchCurrentDayReport = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/hourly-material-report/current-day`);
      if (!response.ok) {
        throw new Error(`Failed to fetch current day data (Status: ${response.status})`);
      }
      const json: HourlyMaterialReportResponse = await response.json();
      setReportData(json.data || []);
    } catch (err: any) {
      setError(err.message || "An error occurred while fetching the hourly report.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApplyFilter = async (e: React.FormEvent) => {
    ...
    if (!selectedDate) {
      setError("Please select a date to filter the hourly report.");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/hourly-material-report/custom-day?report_date=${selectedDate}`
      );

      if (!response.ok) {
        const errJson = await response.json();
        throw new Error(errJson.detail || `Failed to fetch hourly data (Status: ${response.status})`);
      }
  ...
  // Calculate vertical column totals dynamically across time slots
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-slate-500 mt-0.5">
              Cell Chemistry (BAK / LG) Hourly Production Shift Breakdown (07:00 - 05:00)
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold px-3.5 py-1.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200/80 shadow-2xs">
            Mode: {activeFilterMode === "current_day" ? "Today's Shift (Default)" : `Date: ${selectedDate}`}
          </span>
          <button
        ...
        <table className="w-full text-xs text-left border-collapse font-['Open_Sans',sans-serif]">
          ...
          <thead className="bg-slate-100/90 text-slate-800 sticky top-0 z-20 border-b-2 border-slate-300 backdrop-blur-xs">
            <tr>
              <th
                rowSpan={2}
                className="p-3.5 border-r-2 border-b-2 border-slate-300 bg-slate-100 sticky left-0 z-30 font-bold min-w-[140px] text-slate-900 text-center tracking-wide"
              >
                Hourly Time Slot
              </th>
          ...
          <tbody className="divide-y divide-slate-200 tabular-nums text-[13px]">
            {loading ? (
              ...
              Array.from({ length: 12 }).map((_, idx) => (
                <tr key={idx} className="animate-pulse">
                  <td className="p-3.5 bg-slate-50 sticky left-0 border-r-2 border-slate-300">
                    <div className="h-3.5 bg-slate-200 rounded w-24 mx-auto"></div>
                  </td>
              ...
              <tr>
                ...
                >
                  No hourly records found for the selected date.
                </td>
              </tr>
            ) : (
              reportData.map((row) => (
                <tr key={row.hourly_interval} className="hover:bg-slate-50/80 transition-colors">
                  {/* HOURLY TIME SLOT (Sticky Left Column) */}
                  ...
                  {MATERIAL_IDS.map((matId) => {
                    ...
                    return (
                      <React.Fragment key={`${row.hourly_interval}-${matId}`}>
